...
async def file_load(request):
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            # df.get(["student_school_year", "student_class", "student_id",'student_name','birthday','gender','email']) 
            # result = df.to_json(orient='records')
            alchemyEngine = create_engine('postgresql://icangteen_user:123456abcA@localhost:5432/icangteen', pool_recycle=3600);
            alchemyEngineconn = engine.connect()

            for ind in df.index:
                # User.user_name = df['student_id']
                # User.email = df['email']   
                # User.full_name = df['student_name']
                user = User.insert().values
                user = User.insert().values(User.user_name = df['student_id'],User.email = df['email'],User.full_name = df['student_name'],User.extra_data = df, User.company_id=company_id)
                result = conn.execute(user)


            
            # print(result)
            # q = db.session.query(User).with_for_update(nowait=True, of=User)
            # user_name = 
            # full_name
            # email
            # companyid
